...
/**
 * @brief Draws the menu to the oled buffer
 * 
...
void oled_menu_init(void) {
    ...
    main_menu.elements[0] = _menu_create_element("Play Game", 
                                _menu_create_menu_ptr_action(&difficulty_menu));
    main_menu.elements[1] = _menu_create_element("Highscores", 
                                _menu_create_menu_ptr_action(&highscore_menu));
    main_menu.elements[2] = _menu_create_element("Settings", 
                                _menu_create_menu_ptr_action(&settings_menu));
    main_menu.elements[3] = _menu_create_element("Play Star Wars", 
                                _menu_create_func_ptr_action(&_play_star_wars));
    ...
    for (uint8_t i = 0; i < NUM_HIGHSCORE_ELEMENTS; ++i) {
        ...
        highscore_menu.elements[i] = _menu_create_element("", 
                                        _menu_get_empty_action()); 
    }
    ...
    settings_menu.elements[0] = _menu_create_element("Calibrate position", 
                                    _menu_create_func_ptr_action(&_send_calibrate));
    settings_menu.elements[1] = _menu_create_element("Reset highscores", 
                                    _menu_create_func_ptr_action(&_reset_highscores_and_update));
...
void oled_menu_update(void) {
    ...
    bool should_update = false; // To avoid drawing the menu if not needed

    bool in_playing_menu = p_current_menu == &ingame_menu 
                            || p_current_menu == &game_over_menu;


    // Return to main menu if not playing and no new input is given
    // Don't return if in playing menu, as we might want to update the score
    ...
    if (p_current_menu == &ingame_menu) {
        ...
        if (prev_score != new_score || new_score == 0) {
            ...
            sprintf(ingame_menu.elements[1].element_text, "Score: %i", 
                    new_score);
            should_update = true;
        }
        ...
        if (!game_is_playing() || buttons.left && buttons.right) {
            ...
            sprintf(game_over_menu.elements[1].element_text, "Score: %i", 
                    new_score);
            p_current_menu = &game_over_menu;
            should_update = true;
            game_stop();
